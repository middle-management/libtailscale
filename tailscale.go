...
func TsnetListenPacket(sd C.int, network, addr *C.char, listenerOut *C.int) C.int {
	...
	pl := &packetListener{s: s, pc: pc, fd: goFd}
	...
	cleanup := func() {
		cleanupOnce.Do(func() {
			packetListeners.mu.Lock()
			if cur, ok := packetListeners.m[fdC]; ok && cur.pc == pc {
				delete(packetListeners.m, fdC)
				syscall.Close(goFd)
			}
			packetListeners.mu.Unlock()
			pc.Close()
	...
	// datagram. Closing pc (in cleanup) breaks the ReadFrom loop.
	go func() {
		defer cleanup()
		var buf [1 << 16]byte
		for {
